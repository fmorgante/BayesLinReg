...
.subset_blm_ld_eigen <- function(ld, selected) {
  ...
  for (block in ld$blocks) {
    ...
    if (length(keep) == block$size) {
      ...
      complete[block$name] <- TRUE
      next
    }
    weighted <- sweep(
      block$eigenvectors[keep, , drop = FALSE],
      2L, sqrt(block$eigenvalues), `*`
    )
    if (nrow(weighted) <= ncol(weighted)) {
      decomposition <- eigen(tcrossprod(weighted), symmetric = TRUE)
      vectors <- decomposition$vectors
      values <- decomposition$values
    } else {
      small <- eigen(crossprod(weighted), symmetric = TRUE)
      tolerance <- sqrt(.Machine$double.eps) *
        max(1, max(abs(small$values)))
      positive <- small$values > tolerance
      values <- small$values[positive]
      vectors <- sweep(
        weighted %*% small$vectors[, positive, drop = FALSE],
        2L, sqrt(values), `/`
      )
    }
    ...
    vectors <- vectors[, positive, drop = FALSE]
    if (!length(values)) {
      stop(sprintf(
        "Harmonization removed the retained eigenspace of LD block `%s`.",
        block$name
      ), call. = FALSE)
    }
    rownames(vectors) <- ld$variants$ID[global[keep]]
    block$size <- as.integer(length(keep))
  ...
  if (!length(new_blocks)) stop("LD eigen subset is empty.", call. = FALSE)
  variants <- ld$variants[selected_flag, , drop = FALSE]
  rownames(variants) <- NULL
  result <- .new_blm_ld_eigen(
    new_blocks, variants,
    unique(vapply(new_blocks, `[[`, character(1), "parent")),
    ld$requested_prop_var, NULL
  )
  if (!is.null(ld$regularization_report)) {
    rows <- match(names(new_blocks), ld$regularization_report$block)
    if (anyNA(rows)) {
      stop("`ld` regularization metadata are inconsistent.", call. = FALSE)
    }
    report <- ld$regularization_report[rows, , drop = FALSE]
    if (!"source_block" %in% names(report)) report$source_block <- report$block
    report$predictors <- vapply(new_blocks, `[[`, integer(1), "size")
    numerical <- intersect(c(
      "minimum_eigenvalue_before", "minimum_eigenvalue_after",
      "positive_definite_after"
    ), names(report))
    report[!unname(complete[names(new_blocks)]), numerical] <- NA
