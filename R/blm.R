...
#'
#' @details Random draws use R's current random-number stream. Call [set.seed()]
#'   before fitting when reproducible results are required. Multiple chains
#'   use parallel-safe streams generated by the `future` package.
#'
#'   `ETA` may be a single-block specification such as
...
blm <- function(y, ETA, residual_var = NULL,
  ...
  layout <- .prepare_block_layout(blocks)
  x <- do.call(cbind, lapply(blocks, `[[`, "x"))
  colnames(x) <- layout$internal_names
  fixed_indices <- .fixed_predictor_indices(blocks, layout$block_indices)
  ...
  samples <- .run_prepared_sampler(
    sampler_arguments, version, nchains, layout$block_model,
    verbose, iterations
  )

  .assemble_blm_result(
    blocks, layout$block_indices, samples, nchains, store_samples,
